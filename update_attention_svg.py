...
# Changed viewBox to add padding at the top (-30) and bottom (+30)
svg.append('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -40 1100 840" width="100%" height="100%">')
svg.append('<defs>')
...
svg.append('<marker id="arrow-vec" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="#333"/></marker>')
svg.append('</defs>')
...
def draw_arrow(x1, y1, x2, y2, color="#333", dashed=False, marker=None):
    ...
    svg.append(f'<line x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" {style} marker-end="{m}"/>')
...
def draw_vector_label(x, y, char, sub="", size=26, color="#000"):
    # Base char
    svg.append(f'<text x="{x}" y="{y}" font-family="serif" font-size="{size}px" font-weight="bold" font-style="italic" fill="{color}" text-anchor="middle">{char}</text>')
    # Subscript
    if sub:
        svg.append(f'<text x="{x+10}" y="{y+8}" font-family="serif" font-size="{size-8}px" font-weight="normal" font-style="italic" fill="{color}" text-anchor="start">{sub}</text>')
    # Vector arrow
    svg.append(f'<line x1="{x-6}" y1="{y-size+4}" x2="{x+6}" y2="{y-size+4}" stroke="{color}" stroke-width="1.5" marker-end="url(#arrow-vec)"/>')
...
for i in range(4):
    ...
    # Vector label right next to the blue box
    draw_vector_label(cx + box_w + 30, y_rnn2 + box_h/2 + 5, "h", sub="j", size=22, color="#0055aa")
...
draw_vector_label(ctx_cx + box_w + 25, y_ctx + box_h/2 + 8, "c", sub="i", size=24, color="#a67b27")
...
for i in range(4, 8):
    ...
    if i == 4:
        # First decoder cell s_{i-1}
        draw_vector_label(cx + box_w + 35, y_rnn2 + box_h/2 + 5, "s", sub="i-1", size=22, color="#8b0000")
    ...
    draw_arrow(cx + box_w/2, y_att_v - 5, cx + box_w/2, y_out_text + 25, color="#9932CC")
    draw_text(cx + box_w/2, y_out_text + 10, out_words[i-4])
...
draw_arrow(start_x + 5*dx - 10, y_out_text + 20, start_x + 5*dx - 20, y_in_text - 35, dashed=True, color="#B8860B")
draw_arrow(start_x + 6*dx - 10, y_out_text + 20, start_x + 6*dx - 20, y_in_text - 35, dashed=True, color="#B8860B")
draw_arrow(start_x + 7*dx - 10, y_out_text + 20, start_x + 7*dx - 20, y_in_text - 35, dashed=True, color="#B8860B")
...
with open('figures/attention_custom.svg', 'w', encoding='utf-8') as f:
    f.write('\n'.join(svg))

print("Created attention_custom.svg with vectors and viewBox padding")
